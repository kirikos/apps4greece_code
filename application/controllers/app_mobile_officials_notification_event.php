...
class App_mobile_officials_notification_event extends CI_Controller {
	...
	public function index()
	{
		...
		$events= $this->admin_actions_model->get_officials_notifications($day_before);
		...
		while($i<$length_events){
			...
			if($events[$i]['whole_area_officials']==1){
				$track = $xml->addChild('article');
				$track->addChild('id',$events[$i]['id']);
				$track->addChild('photo_reportaz_id	',$events[$i]['photo_reportaz_id']);
				$track->addChild('description',$events[$i]['description_officials']);
				$track->addChild('published_date',$events[$i]['published_date_officials']);
			}else{
				
				$distance = $events[$i]['distance_officials'];
