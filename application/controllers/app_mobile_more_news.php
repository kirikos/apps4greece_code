...
class App_mobile_more_news extends CI_Controller {
	...
	public function index()
	{
		...
		if(!isset($_GET['userid']) || !isset($_GET['category']) || !isset($_GET['offset'])){
		...
		if($offset<2){
			...
			$track = $xml->addChild('violation','offset value');
			Header('Content-type: application/xml');
			print($xml->asXML());
			return;
		...
		$news = $this->article_model->get_mobile_news($category,$offset);
